...
object QT{
  def apply() = QTNil
  def apply(c: List[PieceShell]) = new QTImpl(c,Rect(World.X_MIN,World.Y_MIN,World.X_MAX,World.Y_MAX))
  def apply(c: List[PieceShell], r: Rect) = new QTImpl(c,r)
...
trait QT {
  ...
  val tl,tr,bl,br: QT
  def sub(p: Point): QT = (p.x>rect.center.x,p.y>rect.center.y) match {case (false,false)=>tl case (true,false)=>tr case (false,true)=>bl case (true,true)=>br}
